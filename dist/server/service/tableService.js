...
let TableService = class TableService {
    // probably in the future it is best to put table information
    // and game state information into one ServerState object
    // However, table state changes much less frequently than game state
    ...
    getConnectedClient(cookie, creationAllowed) {
        const client = this.table.activeConnections[cookie];
        if (client) {
            return client;
        }
        else if (creationAllowed) {
            const newClient = this.createConnectedClient(cookie);
            this.table.activeConnections[cookie] = newClient;
            return newClient;
        }
        else {
    ...
    associateClientAndPlayer(connectedClient, player) {
        ...
        connectedClient.gamePlayer = player.uuid;
    }
};
