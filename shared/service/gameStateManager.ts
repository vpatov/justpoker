...
export class GameStateManager {
  ...
  getNumberPlayersSitting() {
    return Object.entries(this.gameState.players).filter(
      ([uuid, player]) => player.sitting
    ).length;
  }
  ...
  isSeatTaken(seatNumber: number) {
    return Object.entries(this.gameState.players).some(
      ([uuid, player]) => player.seatNumber === seatNumber
    );
  }

  isValidSeat(seatNumber: number) {
    return (
      seatNumber >= 0 && seatNumber < this.gameState.gameParameters.maxPlayers
    );
  }
  ...
  dealCardsToBoard(amount: number) {
    const newCards = [...Array(amount).keys()].map((_) =>
      this.deckService.drawCard(this.getDeck())
    );
    this.gameState = {
      ...this.gameState,
  ...
  initializeFlop() {
    ...
    this.setCurrentPlayerToAct(
      this.getNextPlayerInHandUUID(this.getDealerUUID())
    );
  ...
  getNextBettingRoundStage() {
    ...
    assert(
      bettingRoundStage !== BettingRoundStage.SHOWDOWN,
